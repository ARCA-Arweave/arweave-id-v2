...
describe('Test arweaveID.ts functions',()=>{
	...
	it('retrieveArweaveIdfromAddress gets a wallet\'s name for an account with a V2 ID', async () => {
		...
		let res: ArweaveID.ArweaveId = await ArweaveID.retrieveArweaveIdFromAddress('aoaJNC8NcKVfgwaUj6kyJi2hKrVGUsRHCGf8RhKnsic', arweave)
	...
	// it('setArweaveData accepts name & PNG avatar, & returns valid ISetReturn', async () => {
	// 	expect(1)
	...
	// 	let res: ArweaveID.ISetReturn = await ArweaveID.setArweaveData(aridData,jwk,arweave)
	...
	// }, 20000)
	it('setArweaveData accepts name & SVG avatar, & returns valid ISetReturn', async () => {
		expect(1)
		let address = await arweave.wallets.jwkToAddress(jwk)
		let aridData: ArweaveID.ArweaveId = {
			name: 'Testy Mc Testface 2',
